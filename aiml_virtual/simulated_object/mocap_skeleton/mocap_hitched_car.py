    ...
    def update(self) -> None:
        ...
        if self.source is not None:
            ...
            if self.car.mocap_name in mocap_frame and self.trailer.mocap_name in mocap_frame:
                CAR_COG_TO_HITCH = 0.2
                DRAWBAR_LENGTH = 0.18
                TRAILER_COG_TO_HITCH = 0.2
                ...
                hitch_pos = car_pos + car_rot.apply([-CAR_COG_TO_HITCH, 0, 0])
                # the trailer's attachment point is at the front of the trailer
                trailer_front_pos = trailer_pos + trailer_rot.apply([TRAILER_COG_TO_HITCH, 0, 0])
                ...
                ideal_rod_vec = rod_vec/np.linalg.norm(rod_vec) * DRAWBAR_LENGTH # vector between attachment points if the bar is the correct length
                ideal_trailer_front_pos = hitch_pos - ideal_rod_vec # trailer-drawbar connection point if the bar is the correct length
                ideal_trailer_pos = ideal_trailer_front_pos - trailer_rot.apply([TRAILER_COG_TO_HITCH, 0, 0])  # trailer COG if the bar is the correct length
