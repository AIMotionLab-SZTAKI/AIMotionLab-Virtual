...
class MPCCCar(controlled_object.ControlledObject):
    """
    Class that encapsulates an mpcc car along with its actuators and dynamics.
    """
    ...
    def __init__(self, has_trailer: bool = False):
        super().__init__()
        self.update_frequency: float = MPCCCar.CTRL_FREQ  #: The frequency at which the update process runs.
        self.wb: float = 2 * MPCCCar.WHEEL_Y  #: Distance between wheels in the Y direction.
        self.tw: float = 2 * MPCCCar.WHEEL_X  #: Distance between wheels in the X direction.
        self.sensors: dict[str, np.array] = {}  #: Dictionary of sensor data.
        self.wheels: dict[str, Wheel] = {
            "front_left": Wheel(self.name + Wheel.FRONT_LEFT, pos=f"{MPCCCar.WHEEL_X} {MPCCCar.WHEEL_Y} 0"),
            "rear_left": Wheel(self.name + Wheel.REAR_LEFT, pos=f"-{MPCCCar.WHEEL_X} {MPCCCar.WHEEL_Y} 0"),
            "front_right": Wheel(self.name + Wheel.FRONT_RIGHT, pos=f"{MPCCCar.WHEEL_X} -{MPCCCar.WHEEL_Y} 0"),
            "rear_right": Wheel(self.name + Wheel.REAR_RIGHT, pos=f"-{MPCCCar.WHEEL_X} -{MPCCCar.WHEEL_Y} 0")
    ...
    def set_torques(self, d: float) -> None:
        ...
        torque = MPCCCar.C_M1 * d - MPCCCar.C_M2 * v - MPCCCar.C_M3 * np.sign(v)
        for wheel in self.wheels.values():
            wheel.ctrl[0] = utils_general.clamp(torque, MPCCCar.MAX_TORQUE)
    ...
    def get_trailer(self) -> tuple[ET.Element, list[ET.Element]]:
        ...
        ET.SubElement(trailer_wheelrr, "joint", type="hinge", axis="0 1 0", frictionloss=TRAILER.FRICTIONLOSS,
                      damping=TRAILER.DAMPING,
    ...
    def create_xml_element(self, pos: str, quat: str, color: str) -> dict[str, list[ET.Element]]:
        ...
        # Instead of a free joint like this: ET.SubElement(car, "joint", name=self.name, type="free")
        # we have 3 slide joints and 6 hinge joints
        # Order of joints: slide: xyz; hinge: zyx
        ######################### CREATING THE JOINTS FOR A 6DOF SYSTEM ################################################
        ET.SubElement(car, "inertial", pos="0 0 0", diaginertia=MPCCCar.DIAGINERTIA, mass=MPCCCar.MASS)

        ET.SubElement(car, "joint", name=self.name + "slide_x", type="slide",
                      axis="1 0 0")  # This must be replaced with slide joints and hinge joints
        ET.SubElement(car, "joint", name=self.name + "slide_y", type="slide",
                      axis="0 1 0")  # This must be replaced with slide joints and hinge joints
        ET.SubElement(car, "joint", name=self.name + "slide_z", type="slide", axis="0 0 1",
                      ref="0.05")  # This must be replaced with slide joints and hinge joints

        ET.SubElement(car, "joint", name=self.name + "hinge_z", type="hinge",
                      axis="0 0 1")  # This must be replaced with slide joints and hinge joints
        ET.SubElement(car, "joint", name=self.name + "hinge_y", type="hinge",
                      axis="0 1 0")  # This must be replaced with slide joints and hinge joints
        ET.SubElement(car, "joint", name=self.name + "hinge_x", type="hinge",
                      axis="1 0 0")  # This must be replaced with slide joints and hinge joints
        ...
        for wheel in self.wheels.values():
            ...
            if wheel.ctrl_steer is not None and wheel.actr_force_steer is not None:
                pos = wheel.pos.split(" ")
                y = np.sign(float(pos[1])) * MPCCCar.STEER_Y
                ...
                ret["actuator"].append(ET.Element("position", forcelimited="true", forcerange="-5 5",
                                                  name=wheel.name + "_actr_steer", joint=wheel.name + "_steer",
                                                  kp=Wheel.KP))
            ...
            ret["contact"].append(
                ET.Element("pair", geom1=wheel.name, geom2="ground", condim="6", friction=Wheel.FRICTION))
        ...
        if self.has_trailer:
            ...
            ret["sensor"].append(
                ET.Element("gyro", site=self.name + "_trailer_top_plate", name=self.name + "_trailer_ang_vel"))
            ret["sensor"].append(ET.Element("framelinvel", objtype="site", objname=self.name + "_trailer_top_plate",
                                            name=self.name + "_trailer_vel"))
            ret["sensor"].append(ET.Element("framepos", objtype="site", objname=self.name + "_trailer_top_plate",
                                            name=self.name + "_trailer_pos"))
            ret["sensor"].append(ET.Element("framequat", objtype="site", objname=self.name + "_trailer_top_plate",
                                            name=self.name + "_trailer_quat"))
        ############################################### VIRTUAL STATE######################################################

        virtual_body = ET.SubElement(car, "body", name="virtual_state")
        ET.SubElement(virtual_body, "joint", name="virtual_hinge", type="hinge")
        ET.SubElement(virtual_body, "geom", type="box", size="0.02 0.02 0.02", pos="0 0 0.15", contype="0",
                      conaffinity="0")
    ...
    def bind_to_data(self, data: mujoco.MjData) -> None:
        ...
        if self.has_trailer:
            self.data.joint(self.name + "car_to_rod").qpos = utils_general.quaternion_from_euler(0,
                                                                                                 TRAILER.DRAWBAR_DEFAULT_PITCH,
                                                                                                 0)
