...
def create_shifted_slice(slice_, mirrored_slice, offset_x1, offset_x2, offset_y):
    cube_size = slice_.shape[0]
    scale = 10
    slice_upscaled = np.empty((cube_size * scale, cube_size * scale))
    slice_upscaled_mirrored = np.empty_like(slice_upscaled)
    ...
    for si in range(cube_size * scale):
        ...
        for sj in range(cube_size * scale):
            ...
            slice_upscaled_mirrored[si, sj] = mirrored_slice[i, j]
            if((sj + 1) % scale == 0):
                j += 1 
...
if USE_EXISTING_DATA:
    ...
    # TODO: FILE NAME HAS TO BE SET
    mirrored_data_file_name = os.path.join(abs_path, "..", "airflow_data", "raw_airflow_data", "single_rotor_pressure.csv")
    ...
    mirrored_data = np.reshape(mirrored_tmp[:, 3], (cube_size, cube_size, cube_size))
    print(data.shape)
    ...
    for i in range(cube_size):
        ...
        slice_shifted = create_shifted_slice(data[:, :, i], mirrored_data[:, :, i], offset_x1, offset_x2, offset_y)
