...
if __name__ == "__main__":
    ...
    csv_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', 'csv', 'paperclip4.csv')
    meas_state, meas_time, meas_data = load_meas_data(csv_path)

    if trajectory_type == 'paperclip':
        path_points = np.vstack((paperclip(), paperclip()[1:, :]))
    ...
    car_trajectory.build_from_points_const_speed(path_points=path_points, path_smoothing=1e-4, path_degree=5,
                                                        const_speed=0.6)
    predictor = TrailerPredictor(car_trajectory, payload_type=PAYLOAD_TYPES.Box)
    load_pos, _, _ = predictor.simulate(meas_state[0, :], 0, meas_time[-1], predicted_obj)
