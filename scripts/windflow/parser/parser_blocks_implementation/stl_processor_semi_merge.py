    ...
    def _get_intersection_group(self, target_mesh_index):
        ...
        while group_index != len(group):
            ...
            for i in range(len(self._meshes)):
                ...
                do_meshes_intersect_by_bounding_box = self._do_meshes_intersect_by_bounding_box(self._meshes[i], self._meshes[current_mesh_index])
                if not do_meshes_intersect_by_bounding_box:
                    continue

                do_meshes_intersect_precisely = self._do_meshes_intersect_precisely(self._meshes[i], self._meshes[current_mesh_index])
